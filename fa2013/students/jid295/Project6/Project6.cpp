...
void processPurchase() {
    ...
    readString(item);
    readNum(count);
    item_chosen = selectInventItem(item);
    /* Ensure items were requested. */
    if (count == 0) { return; }
    /* Ensure a real item was asked for. */
    if (item_chosen) {
        if (*selectInventItem(item) >= count) {
            *selectInventItem(item) -= count;
            *selectInventItem(item, database[customer_name]) += count;
        } else {
            /* Sorry __NAME__, we only have __NUMBER__ __ITEMS__ */
            printf("Sorry %s, we only have %d %s\n",
                    customer_name.c_str(),
                    *selectInventItem(item),
                    item.c_str());
        }
    }
}
...
void processSummarize() {
    ...
    printf("we have had a total of %d different customers\n",
           database.length);
    /* Find the maximum values for each item. */
    Customer *max;
    if ((max = findMax("Diapers"))) {
